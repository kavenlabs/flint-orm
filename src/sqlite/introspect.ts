...
const TEXT_TYPES = new Set([
  "text", "varchar", "character", "char", "clob", "native character",
  "nvarchar", "nchar", "nyclob", "numeric",
]);

const INTEGER_TYPES = new Set([
  "integer", "int", "smallint", "tinyint", "bigint", "unsigned big int",
  "int2", "int8", "mediumint",
]);
...
export function introspect(client: Database): SchemaState {
  ...
  const tableRows = client
    .query(
      `SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' AND name != '__flint_migrations'`,
    )
    .all() as { name: string }[];
