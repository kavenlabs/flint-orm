 ...
 */
export function count<T extends AnyTable>(client: Database, table: T, condition?: Condition): number {
 ...
 */
export function countColumn<T extends AnyTable, C extends ColumnDef<any, any>>(client: Database, table: T, column: C, condition?: Condition): number {
  const tableName = getTableName(table);
 ...
 */
export function sum<T extends AnyTable, C extends ColumnDef<any, any>>(client: Database, table: T, column: C, condition?: Condition): number | null {
  const tableName = getTableName(table);
  const columnName = getColumnName(column);
  const { whereSql, params } = compileWhere(condition);
  const sql = `SELECT sum(${columnName}) as total FROM ${tableName}${whereSql}`;
  const result = client.prepare(sql).get(...(params as SQLQueryBindings[])) as {
    total: number | null;
  };
  return result.total;
}
 ...
 */
export function avg<T extends AnyTable, C extends ColumnDef<any, any>>(client: Database, table: T, column: C, condition?: Condition): number | null {
  const tableName = getTableName(table);
  const columnName = getColumnName(column);
  const { whereSql, params } = compileWhere(condition);
 ...
 */
export function min<T extends AnyTable, C extends ColumnDef<any, any>>(client: Database, table: T, column: C, condition?: Condition): number | null {
  const tableName = getTableName(table);
  const columnName = getColumnName(column);
  const { whereSql, params } = compileWhere(condition);
 ...
 */
export function max<T extends AnyTable, C extends ColumnDef<any, any>>(client: Database, table: T, column: C, condition?: Condition): number | null {
  const tableName = getTableName(table);
  const columnName = getColumnName(column);
  const { whereSql, params } = compileWhere(condition);
