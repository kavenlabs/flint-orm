...
const joinResult = db.leftJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).execute();
...
const joinColResult = db.leftJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).columns(['id', 'total']).execute();
...
const joinSingleResult = db.leftJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).single().execute();
