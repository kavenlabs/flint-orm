...
db.batch([db.insert(orders).values({ id: 'o1', userId: 'u2', total: 100 }), db.insert(orders).values({ id: 'o2', userId: 'u2', total: 250 })]);
...
const ordersWithItems = db.leftJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).execute();
...
const ordersNarrowed = db.leftJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).columns(['id', 'total']).execute();
...
const innerJoined = db.innerJoin(orders).on(orderItems, eq(orders.id, orderItems.orderId)).execute();
...
const singleName = db.select().from(users).columns(['name']).where(eq(users.id, 'u2')).single().execute();
...
const [count2, sum2, avg2] = await Promise.all([db.count(orders), db.sum(orders, orders.total), db.avg(orders, orders.total)]);
